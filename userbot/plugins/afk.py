...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await borg.send_message(event.chat_id, f"**     ⚠️ AT THE MOMENT I'M OFFLINE.**\n**SO PLEASE DON'T SPAM IN MY CHAT, THANKS 🌈** \n**I'LL AWNSER AS SOON AS POSIBLE! \n〽️** __REASON ~ {reason}__")
        else:
            await borg.send_message(event.chat_id, f"**🔒 __i'm now AFK__**")
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"**     ⚠️ AT THE MOMENT I'M OFFLINE.**\n**SO PLEASE DON'T SPAM IN MY CHAT, THANKS 🔥** \n**I'LL AWNSER AS SOON AS POSIBLE!" + \
                           f"\n☀️** __AFK FOR ~ `{total_afk_time}`__ \n__✨ REASON ~ {reason}__" \
                           if reason \
                           else f"**     ⚠️ AT THE MOMENT I'M OFFLINE.**\n**SO PLEASE DON'T SPAM IN MY CHAT, THANKS 🔥** \n**I'LL AWNSER AS SOON AS POSIBLE! \n☀️** __AFK FOR ~ `{total_afk_time}`__"
